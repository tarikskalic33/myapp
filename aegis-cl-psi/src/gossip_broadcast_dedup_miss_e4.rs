...
//! HIGH_DEDUP_MISS_E4_THRESHOLD = 11: dedup_miss_rate_pct > 11 → high_dedup_miss_e4

use sha2::{Sha256, Digest};

pub const DEDUP_MISS_E4_GENESIS_HASH: [u8; 32] = [0u8; 32];
pub const HIGH_DEDUP_MISS_E4_THRESHOLD: u32 = 11;

#[derive(Debug, Clone, PartialEq)]
pub struct GossipDedupMissE4Entry {
    pub epoch_end:          u64,
    pub dedup_misses:       u32,
    pub total_lookups:      u32,
    pub dedup_miss_rate_pct: u32,
    pub high_dedup_miss_e4: bool,
...
fn compute_hash(
    ...
    total_lookups: u32,
    rate_pct: u32,
    high_dedup_miss_e4: bool,
) -> [u8; 32] {
    let mut h = Sha256::new();
    h.update(prev);
    h.update(epoch_end.to_be_bytes());
    h.update(dedup_misses.to_be_bytes());
    h.update(total_lookups.to_be_bytes());
    h.update(rate_pct.to_be_bytes());
    h.update([high_dedup_miss_e4 as u8]);
    h.finalize().into()
}

pub struct GossipDedupMissE4Log {
    pub entries: Vec<GossipDedupMissE4Entry>,
}

impl GossipDedupMissE4Log {
    pub fn new() -> Self {
    ...
    pub fn record(
        ...
        dedup_misses: u32,
        total_lookups: u32,
    ) -> &GossipDedupMissE4Entry {
        let denom = total_lookups.max(1) as u64;
        let dedup_miss_rate_pct =
            ((dedup_misses as u64).saturating_mul(100) / denom).min(100) as u32;
        let high_dedup_miss_e4 = dedup_miss_rate_pct > HIGH_DEDUP_MISS_E4_THRESHOLD;
        let prev = self.entries.last().map(|e| e.entry_hash).unwrap_or(DEDUP_MISS_E4_GENESIS_HASH);
        ...
        self.entries.push(GossipDedupMissE4Entry {
            epoch_end,
            dedup_misses,
            total_lookups,
            dedup_miss_rate_pct,
    ...
    pub fn high_dedup_miss_e4_count(&self) -> usize {
        self.entries.iter().filter(|e| e.high_dedup_miss_e4).count()
    }
    ...
    pub fn verify_chain(&self) -> (bool, Option<usize>) {
        let mut prev = DEDUP_MISS_E4_GENESIS_HASH;
...
impl Default for GossipDedupMissE4Log {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn record_fields_correct_flag_true_when_above_threshold() {
        let mut log = GossipDedupMissE4Log::new();
        let e = log.record(1000, 15, 100);
        assert_eq!(e.epoch_end, 1000);
        assert_eq!(e.dedup_misses, 15);
        assert_eq!(e.total_lookups, 100);
        assert_eq!(e.dedup_miss_rate_pct, 15);
        assert!(e.high_dedup_miss_e4);
    }

    #[test]
    fn flag_false_when_exactly_at_threshold() {
        let mut log = GossipDedupMissE4Log::new();
        // rate = (11 * 100) / 100 = 11, not > 11
        let e = log.record(2000, 11, 100);
        assert_eq!(e.dedup_miss_rate_pct, 11);
        assert!(!e.high_dedup_miss_e4);
    }
    ...
    #[test]
    fn total_lookups_zero_no_div_by_zero() {
        let mut log = GossipDedupMissE4Log::new();
    ...
    fn threshold_constant_value_is_11() {
        assert_eq!(HIGH_DEDUP_MISS_E4_THRESHOLD, 11);
    }
    ...
    fn first_prev_hash_equals_genesis() {
        ...
        assert_eq!(e.prev_hash, DEDUP_MISS_E4_GENESIS_HASH);
    }
    ...
    fn verify_chain_empty_returns_true_none() {
        let log = GossipDedupMissE4Log::new();
    ...
    fn determinism_same_inputs_same_hash() {
        let mut log1 = GossipDedupMissE4Log::new();
        let mut log2 = GossipDedupMissE4Log::new();
        let mut log3 = GossipDedupMissE4Log::new();
    ...
    #[test]
    fn high_dedup_miss_e4_count_mixed_log() {
        let mut log = GossipDedupMissE4Log::new();
        ...
        assert_eq!(log.high_dedup_miss_e4_count(), 2);
    }
    ...
    fn default_produces_zero_entries() {
        let log = GossipDedupMissE4Log::default();
